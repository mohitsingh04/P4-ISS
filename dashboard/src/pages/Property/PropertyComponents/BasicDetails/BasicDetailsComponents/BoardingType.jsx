...
export default function BoardingType({ property, getProperty }) {
    ...
    try {
      ...
      setCategory(
        data.filter(
          (item) =>
            item.status === "Active" &&
            item.parent_category.toLowerCase() === "boarding type"
        )
      );
    } catch (error) {
      console.error(
  ...
  const formik = useFormik({
    initialValues: {
      boarding_type: property?.boarding_type?.toString() || "",
    },
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        const errorMsg =
          error?.response?.data?.message || "Failed to update Boarding type.";
      ...
      <Form.Group>
        ...
        {!isUpdating ? (
          <div className="input-group">
            <Form.Control
              value={getCategoryToRelatedId(property?.boarding_type) || "N/A"}
              disabled
            />
          ...
          <Form onSubmit={formik.handleSubmit}>
            <div className="input-group">
              <Form.Select
                name="boarding_type"
                value={formik.values.boarding_type}
                onChange={formik.handleChange}
                isInvalid={
                  formik.touched.boarding_type && !!formik.errors.boarding_type
                }
              >
