import { PropertyProps, ReviewProps } from "@/types/types";
...
const ReviewsTab = ({
  ...
  const onSubmit = async () => {
    await getProperty(property?.uniqueId);
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Fixed fallback logic - ensuring proper condition checking */}
      {showReviewForm ? (
        ...
        <div className="space-y-6">
          {safeReviews.map((review, index) => (
            <div
              key={index}
              className="bg-white rounded-2xl shadow-xs p-6 hover:shadow-sm transition-shadow duration-300"
            >
