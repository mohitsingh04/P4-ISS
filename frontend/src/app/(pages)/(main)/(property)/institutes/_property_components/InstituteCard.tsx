    ...
    >
      ...
      <div className="flex flex-col p-4 md:p-6 flex-1">
        ...
        <div className="flex justify-between items-start mb-2">
          <Link href={slug}>
            <h3 className="text-lg sm:text-xl font-semibold text-gray-900 hover:text-indigo-600 line-clamp-2">
        ...
        <div className="flex items-center text-gray-600 text-sm mb-3 gap-2">
          <LuGraduationCap className="w-4 h-4 flex-shrink-0 text-indigo-500" />
          <span className="capitalize">{institute.category}</span>
          <Link
            href={`/compare/${institute?.property_slug}`}
            className="ml-auto text-xs sm:text-sm px-3 py-1 border border-indigo-600 text-indigo-700 rounded-lg hover:bg-indigo-600 hover:text-white transition duration-200"
          >
            Compare
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-gray-600 mb-4">
          <div className="flex items-center">
            <LuAward className="w-4 h-4 mr-2 text-indigo-500" />
            <span className="line-clamp-1">{institute.property_type}</span>
          </div>
          {institute.est_year && (
            <div className="flex items-center">
              <LuUser className="w-4 h-4 mr-2 text-indigo-500" />
        ...
        <Link
          href={slug}
          className="mt-auto block w-full text-center bg-gradient-to-r from-indigo-600 to-indigo-700 text-white py-3 rounded-xl hover:from-indigo-700 hover:to-indigo-800 hover:scale-[1.02] transition duration-200 font-medium text-sm sm:text-base shadow-lg"
        >
          View Details
        </Link>
      </div>
