        ...
        >
          <div className="flex items-center">
            <input
              ...
              className="appearance-none w-4 h-4 bg-white border-2 r rounded border-white outline outline-indigo-500 checked:bg-indigo-600 transition-all duration-300"
            />
            <div className="ml-2 flex items-center">
              ...
              <span className="ml-2 text-sm text-gray-700 group-hover:text-indigo-600 transition-colors">
                {option.name}
              </span>
            </div>
          </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-semibold text-gray-900 flex items-center">
          <LuFilter className="w-5 h-5 mr-2 text-indigo-600" />
          Filters
        </h3>
      </div>
      ...
      {/* Exam Filters */}
      {dynamicFilterOptions.examsNames.length > 0 && (
        <FilterSection
          title="Exam Name"
          isExpanded={expandedFilters.exam_name}
          onToggle={() => onToggleFilter("exam_name")}
        >
          <CheckboxFilter
            items={dynamicFilterOptions.examsNames}
            filterType="exam_name"
            selectedItems={filters.exam_name}
            searchTerm={filterSearchTerms.exam_name}
            onSearchChange={(value) => onFilterSearchChange("exam_name", value)}
            onFilterChange={
              onCheckboxFilter as (filterType: string, value: string) => void
            }
          />
