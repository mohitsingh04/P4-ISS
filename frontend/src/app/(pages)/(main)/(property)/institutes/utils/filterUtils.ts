...
export const createDynamicFilterOptions = (
          ...
          switch (filterType) {
            ...
            case "exam_name":
              return filterValues.some((name) =>
                institute.exams.some(
                  (exam) =>
                    generateSlug(exam.exam_name || "") === generateSlug(name)
                )
              );
            case "rating":
  ...
  const filteredForExamsNames = getFilteredInstitutesForCount("exam_name");
  const examsNames = [
    ...new Set(
      filteredForExamsNames.flatMap((inst) =>
        inst.exams
  ...
  return {
    ...
    getStatesWithCounts,
    getCitiesWithCounts,
    examsNames: examsNames.map((name) => ({
      name: name,
      count: filteredForExamsNames.filter((inst) =>
        inst.exams.some(
          (exam) => generateSlug(exam.exam_name || "") === generateSlug(name)
        )
      ).length,
    })),
...
export const filterInstitutes = (
  ...
  return institutes.filter((institute) => {
    ...
    const matchesExamsName =
      filters.exam_name.length === 0 ||
      filters.exam_name.some((name) =>
        institute.exams.some(
          (exam) => generateSlug(exam.exam_name || "") === generateSlug(name)
        )
      );
